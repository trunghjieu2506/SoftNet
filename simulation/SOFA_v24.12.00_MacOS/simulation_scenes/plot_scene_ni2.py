...
positions = load_data(r"C:\Users\justi\SOFA\v24.06.00\fingerMonitorA_x.txt")
positionsB = load_data(r"C:\Users\justi\SOFA\v24.06.00\fingerMonitorB_x.txt")
#velocities = load_data(r"C:\Users\justi\SOFA\v24.06.00\fingerMonitor_v.txt")
forces = load_data(r"C:\Users\justi\SOFA\v24.06.00\fingerMonitorA_f.txt")
...
x_pos, y_pos, z_pos = positions[:, 1], positions[:, 2] + 10, positions[:, 3] + 10
x_posB, y_posB, z_posB = positionsB[:, 1], positionsB[:, 2] + 10, positionsB[:, 3] + 10
x_force, y_force, z_force = forces[:, 1], forces[:, 2], forces[:, 3]
# bending_vector = np.array([x_pos, np.zeros_like(x_pos), z_pos]).T
...
magnitudes = np.linalg.norm(bending_vector, axis=1) * 60

# Avoid numerical errors with clip
dot_products = np.einsum('ij,j->i', bending_vector, axis_vector)
theta_xz = np.arccos(dot_products / magnitudes) * (180 / np.pi)
# theta_xz = np.arccos(np.dot(bending_vector, axis_vector)/np.linalg.norm(bending_vector)) * (180 / np.pi)

# Compute torque
torque = np.abs(resultant_force * y_pos * np.sin(np.deg2rad(theta_xz)))
