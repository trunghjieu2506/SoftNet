...
def createScene(rootNode):
    rootNode.addObject('RequiredPlugin', pluginName=['SoftRobots','Sofa.Component.SolidMechanics.FEM.HyperElastic'])
    ...
    finger.addObject('MeshGmshLoader', name='loader', filename='data/mesh/act23mmwithconnector.msh', rotation=[90,0,-90],translation=[22.5,0,0])
    #finger.addObject('MeshGmshLoader', name='loader', filename='data/mesh/act23mmwithconnector075.msh', rotation=[90,0,-90],translation=[22.5,0,0])# use a finer mesh setting
    ...
    finger.addObject('TetrahedronHyperelasticityFEMForceField', materialName="NeoHookean", ParameterSet=str(mu_) + " " + str(k0_))
    #finger.addObject('TetrahedronHyperelasticityFEMForceField',template='Vec3', name="HyperElasticMaterial", materialName="NeoHookean", ParameterSet="3448.2 31034.4")
    ...
    loopnum=8 #number of loops
    int=3 #distance between fiber loops.
    loops, spring_set, indices1, indices2,lengths = looptest(int, loopnum) 
    ...
    hitching.addObject("MechanicalObject", template="Vec3", name="DOF",
                    position=[[(i)*int+1, 0, -4] for i in range(loopnum)],
    ...
    hitching.addObject("StiffSpringForceField", template="Vec3d", name="springs", showArrowSize=0.5, drawMode=1,
                   stiffness=Ks,damping=Kd, indices1=[0,1,2,3,4,5,6], indices2=[1,2,3,4,5,6,7], lengths=int)
    ...
    cavity.addObject('MeshSTLLoader', name='cavityLoader', filename='data/mesh/CHAMBER20mm-075mm.stl', rotation=[90,0,90])
